...
\begin{enumerate}
    ...
    \item \texttt{@Id} --- помечает поле как первичный ключ;
    \item \texttt{@GeneratedValue(strategy = GenerationType.IDENTITY)} --- автоинкремент первичного ключа средствами БД;
    ...
    \item \texttt{@JoinColumn} --- задаёт имя FK-колонки.
\end{enumerate}
...
\subsection*{Связи в моделях:}

\textbf{\texttt{Api -> Organization} (many-to-one):}
...
\textbf{\texttt{VkApi -> VkPost} (one-to-many):}
\begin{lstlisting}[language=Java]
@OneToMany(mappedBy = "vkApi", cascade = CascadeType.ALL, orphanRemoval = true)
private List<VkPost> posts = new ArrayList<>();
\end{lstlisting}

\textbf{\texttt{TgPayload -> TgApi} (many-to-one):}
\begin{lstlisting}[language=Java]
@ManyToOne(optional = false)
...
\textbf{\texttt{TgApi -> TgPayload} (one-to-many):}
\begin{lstlisting}[language=Java]
@OneToMany(mappedBy = "tgApi", cascade = CascadeType.ALL, orphanRemoval = true)
